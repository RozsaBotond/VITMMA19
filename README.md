...
The solution includes robust data preprocessing, augmentation to handle class imbalance, and a comprehensive training pipeline. Training utilizes **Cosine Annealing Learning Rate Scheduling** (`CosineAnnealingLR`). This technique starts with a relatively high learning rate to traverse the loss landscape quickly and then smoothly decays it to a minimum value. This approach helps the model settle into flatter, more robust minima and often leads to better generalization compared to constant learning rates.
...
The project aims for an extra mark through several contributions:
- **Comprehensive Hyperparameter Optimization**: Extensive Bayesian HPO was conducted using Optuna for key models (Transformer, Hierarchical LSTM), leading to significantly improved performance.
- **Robustness and Bug Fixing**: Identification and resolution of critical bugs, such as incorrect test data normalization and mismatched hyperparameters between HPO results and the main training script, demonstrate a deep understanding of the pipeline and ability to debug complex issues.
- **Modular and Containerized Architecture**: The project features a well-structured, modular codebase with a fully functional Docker container for consistent and reproducible execution across environments.
...
The log file must be uploaded to `log/run.log` to the repository. The logs are easy to understand and self-explanatory. `src/utils/utils.py` is used to configure the logger so that output is directed to stdout (which Docker captures).
