...
def convert_labels_to_hierarchical(Y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        Tuple of (stage1_labels, stage2_labels)
    """
    stage1_labels = np.zeros_like(Y)
    stage2_labels = np.zeros_like(Y)
    
    for old_class, new_class in LABEL_7CLASS_TO_STAGE1.items():
        stage1_labels[Y == old_class] = new_class
        
    for old_class, new_class in LABEL_7CLASS_TO_STAGE2.items():
        stage2_labels[Y == old_class] = new_class
...
class HierarchicalBayesianOptimizer:
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        scheduler_type = trial.suggest_categorical("scheduler", ["cosine", "one_cycle", "step"])
        ...
        if use_class_weights:
            class_weight_scale = trial.suggest_float("class_weight_scale", 0.2, 1.0, step=0.2)
        else:
    ...
    def run_optimization(
        ...
        test_results = self._train_final_model(best_trial.params)
        ...
        results_file = f"{save_dir}/hierarchical_bayesian_{timestamp}.json"
    ...
    def _train_final_model(self, params: Dict) -> Dict:
        ...
        for epoch in range(self.epochs):
            ...
            for X_batch, Y_s1, Y_s2, Y_7class in train_loader:
                ...
                loss_s1 = criterion_s1(
                    s1_logits.reshape(-1, 3), Y_s1.reshape(-1)
                )
                loss_s2 = criterion_s2(
                    s2_logits.reshape(-1, 3), Y_s2.reshape(-1)
                )
        ...
        detection_rate = np.mean(all_preds[pattern_mask] > 0) if pattern_mask.sum() > 0 else 0
        ...
        # Save model
        save_dir = "models/hierarchical_v1b"
